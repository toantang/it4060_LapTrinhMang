...
/*
* @function buildBodyRequest : build string format to server
* @param type : a pointer to a type request string
* @param data : data request string
*
* @return :  a string with correct format
*/
char * buildBodyRequest(char * type, char * data) {
	char * rs = (char *)malloc(sizeof(char) * 1024);
	strcpy(rs, type);
	strcat(rs, data);
	strcat(rs, ENDING_DELIMITER);
	return rs;
}

/*
* @function parseResponse : return string result base on response code
* @param buff : a pointer to response code
*
* @return :  a string has infomation
*/
char * parseResponse(char * buff){
...
int main(int argc, char *argv[]) {
	...
	do {
		...
		case 1:
			...
			gets_s(buff, 1024);
			strcpy(buff, buildBodyRequest("USER ", buff));
		...
		case 2:
			...
			gets_s(buff);
			// add prefix "POST" 
			strcpy(buff, buildBodyRequest("POST ", buff));
			printf("Type request: Post message \n");
			break;
		case 3:
			strcpy(buff, buildBodyRequest("QUIT", ""));
		...
		if (ret == SOCKET_ERROR) {
			...
			buff[ret] = '\0';
			char result[1024];
			strcpy(result, parseResponse(buff));
			if (buff[0] == '-') {
